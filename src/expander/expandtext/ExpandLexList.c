...
void	expand_text(t_core *g_core,t_lexlist *lex_table, int is_after_meta) // "$USER", 0 (harf harf ptr ye ekliyoruz)
{
	char	*cmd;
	int		flag;
	char	*ptr;
	...
	while (*cmd) // node'un contenti NULL olmadığı sürece
	{
		if (*cmd == '\'' && (!flag || flag == 1)) // eğer tek tırnak varsa ve (bayrak 0 veya bayrak 1 ise) yani henüz tırnak içinde değilsek
			...
			expand_order(g_core,&ptr, &cmd); // cmd line artık node un sonunu gösteriyor. $USER ile gittik R yi gösteren ptr ile geldik. 
		else
