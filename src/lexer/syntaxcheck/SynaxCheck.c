...
void	syntax_check(t_core *g_core)
{
	...
	while (lex_list)
	{
		syntax_error = syntax_error_check(g_core,lex_list); // -1 gelirse 
		if (syntax_error == -1)
			break ;
...
int	syntax_error_check(t_core *g_core,t_lexlist *lex_list) // 
{
	...
	{
		if (!file_error_check(g_core,lex_list)) //sonraki node text ise 1 geliyor, yoksa veya değilse sıfır geliyor.
			return (-1);
	}
	else if (lex_list->type == SIGN_PIPE) // type'ı PIPE ise
	{
		pipe_error = pipe_error_check(g_core,lex_list); // gelen değer -1 ise return 0 (yukarıda cont atıcak) , gelen değer sıfır ise return -1(yukarıda break atıcak (sorunlu))
		if (pipe_error < 0) 
			return (0);
		else if (!pipe_error)
			return (-1);
